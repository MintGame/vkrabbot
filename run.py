...
jobs = ['Живем Живем', 'Повезло Повезло', '.']
...
def get_slaves_to_best(slaves_list):
    job_best = []

    for slave in slaves_list['slaves']: # Получаем каждого раба в словаре
        if slave['job']['name'] == 'Это принцесска': # если "Это принцесска"
            job_best.append(slave['id']) # добавляем раба в лучших
...
#    while True:
#        slaves_list = get_slaves(local_id)
#        slaves_to_best = get_slaves_to_best(slaves_list)
#        print(f'{len(slaves_to_best)} раба(ов) подлежат сохранению')
        
        # кидаем цепи на рабов
#        for slave in slaves_to_best:
#            fetter(slave)

            # обход блокировки
#            time.sleep((random.randrange(0, 2) * random.randrange(0, 2)) + min(0.9, 145 / len(slaves_to_best)))
#            time.sleep(random.randrange(0, 1))
#        else:
#            time.sleep(random.randrange(11, 18))
